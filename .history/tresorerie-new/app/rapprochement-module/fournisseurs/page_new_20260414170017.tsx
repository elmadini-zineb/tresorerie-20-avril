...
export default function FournisseursPage() {
  ...
  const handleArchive = (id: string) => {
    setFournisseurs(fournisseurs.map((f) => (f.id === id ? { ...f, archived: true } : f)))
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          ...
          <DialogContent>
            ...
            <FournisseurForm initialData={editingFournisseur || undefined} onSave={handleSave} />
          </DialogContent>
        </Dialog>
      </div>
              ...
              {paginatedFournisseurs.length > 0 ? (
                paginatedFournisseurs.map((fournisseur) => (
                  <TableRow key={fournisseur.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <TableCell className="text-[#1B2E5E]">{fournisseur.name}</TableCell>
                    <TableCell className="text-[#64748B]">{fournisseur.email}</TableCell>
                    <TableCell className="text-[#64748B]">{fournisseur.phone}</TableCell>
                    <TableCell className="text-[#64748B]">{fournisseur.address}</TableCell>
                    <TableCell>
                      <Badge variant={fournisseur.active ? 'default' : 'outline'} className={fournisseur.active ? 'bg-[#10B981] text-white' : ''}>
