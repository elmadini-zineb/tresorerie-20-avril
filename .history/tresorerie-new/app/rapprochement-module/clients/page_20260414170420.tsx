...
export default function ClientsPage() {
  ...
  const filteredClients = clients.filter(
    (c) => !c.archived && (c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.email.toLowerCase().includes(searchTerm.toLowerCase()))
  )
  ...
  const handleArchive = (id: string) => {
    setClients(clients.map((c) => (c.id === id ? { ...c, archived: true } : c)))
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          ...
          <DialogContent>
            ...
            <ClientForm initialData={editingClient || undefined} onSave={handleSave} />
          </DialogContent>
        </Dialog>
      </div>
              ...
              {paginatedClients.length > 0 ? (
                paginatedClients.map((client) => (
                  <TableRow key={client.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <TableCell className="text-[#1B2E5E]">{client.name}</TableCell>
                    <TableCell className="text-[#64748B]">{client.email}</TableCell>
                    <TableCell className="text-[#64748B]">{client.phone}</TableCell>
                    <TableCell className="text-[#64748B]">{client.address}</TableCell>
                    <TableCell>
                      <Badge variant={client.active ? 'default' : 'outline'} className={client.active ? 'bg-[#10B981] text-white' : ''}>
