...
import { Clock, CheckCircle, AlertTriangle, Receipt, Eye, ArrowRight, TrendingUp, TrendingDown } from 'lucide-react'
    ...
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kpiCards.map((kpi) => {
            const Icon = kpi.icon
            return (
              <Card key={kpi.title} className="border-[#DDE3EF] bg-white shadow-sm">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    {kpi.trendUp !== null && (
                      <div className={`flex items-center gap-1 text-xs ${kpi.trendUp ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>
                        {kpi.trendUp ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                      </div>
                    )}
                  </div>
                  <div className="mt-3">
                    <p className="text-sm text-[#64748B]">{kpi.title}</p>
                    <p
                      className="text-2xl font-bold"
                      style={{ color: kpi.isAmount ? kpi.color : '#1B2E5E' }}
                    >
                      {kpi.value}
                    </p>
                    <p className="mt-1 text-xs text-[#94A3B8]">{kpi.trend}</p>
                  </div>
                </CardContent>
              </Card>
            )
